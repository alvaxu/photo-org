...
基于AI技术的智能照片管理平台，支持在家庭网络内，通过各种终端，将照片**去重导入**（导入照片时自动去除名字不同的同一张照片）、分析、分类、标签生成、**人脸识别与根据肖像快速查询**、**GPS转地址多服务解析**、**图像特征提取与相似照搜索**、支持对照片及其信息的增删改查、播放和下载。您的照片数据始终存储在您控制的本地设备中，从不上传到云端，既做到了**真正的数据永不离身，绝对隐私无忧，**又做到了**照片随时随地多终端导入和访问**。
...
**导航栏功能：** 页面顶部提供快速入口，包括**导入照片**、**基础分析**、**相似照识别**、**人脸识别**、**GPS转地址**、**AI分析(可选)**等核心功能按钮。
