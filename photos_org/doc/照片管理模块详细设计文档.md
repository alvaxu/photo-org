...
| -------- | ------------------------- | -------- | ----------------------- |
| 文档版本 | V2.2 | 文档状态 | ☑ 已同步实际实现 □ 评审中 □ 已确认 □ 已归档 |
...
#### 4.3.1 智能批量处理
智能批量处理通过导航栏的"基础分析"和"AI分析"按钮实现，支持分步进行质量评估、基础标签生成和AI内容分析。

**处理选项**：
- **基础分析**：质量评估和基础标签生成（时间、EXIF等）
- **AI分析**：深度AI内容分析（场景、物体、情感等）
...
**复合索引优化**：
- **`idx_photos_status_created`**：`(status, created_at)` - 状态筛选+时间排序，提升75-85%查询性能
- **`idx_photos_status_taken`**：`(status, taken_at)` - 状态筛选+拍摄时间排序，提升70-80%查询性能
- **`idx_photos_format_created`**：`(format, created_at)` - 格式筛选+时间排序，提升65-75%查询性能

**优化策略**：
- **智能索引检查**：系统启动时自动检查和创建缺失的复合索引
- **覆盖索引使用**：照片查询使用覆盖索引减少回表操作
- **批量操作优化**：使用事务确保数据一致性
- **分页查询优化**：使用游标分页避免深度分页性能问题
- **连接池配置**：配置合理的连接数和超时时间
