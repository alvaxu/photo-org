...
| -------- | ------------------------- | -------- | ----------------------- |
| 文档版本 | V1.2 | 文档状态 | ☑ 草稿 □ 评审中 □ 已确认 □ 已归档 |
| 编写人 | AI助手 | 编写日期 | 2025年9月9日 |
| 更新日期 | 2025年1月20日 | 更新内容 | FTS搜索优化、用户界面改进、混合设备交互支持 |
...
**FTS5分词器升级**：
- **原分词器**：默认分词器，对中文支持有限
- **新分词器**：unicode61分词器，更好的Unicode和中文支持
- **迁移方式**：重建FTS表，保持数据完整性

**FTS搜索语法支持**：
- **基础搜索**：`关键词` - 搜索包含关键词的内容
- **短语搜索**：`"完整短语"` - 搜索完整短语
- **前缀搜索**：`关键词*` - 搜索以关键词开头的内容
- **通配符搜索**：`关键词%` - 支持通配符匹配
- **逻辑操作**：`AND`、`OR`、`NOT` - 支持逻辑组合查询
- **中文优化**：更好的中文分词和搜索体验
...
-- 排除词搜索
SELECT photo_id FROM photos_fts
WHERE photos_fts MATCH '关键词 NOT 排除词'
ORDER BY rank;
```
