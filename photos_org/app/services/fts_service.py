...
class FTSService:
    ...
    def get_fts_version(self, db: Session) -> int:
        ...
        :param db: 数据库会话
        :return: 版本号 (0=不存在, 1=旧版, 2=包含地址字段, 3=包含AI语义字段)
        """
        try:
            ...
            if 'scene_type' in columns:
                return 3  # V3: 包含AI语义字段
            elif 'location_name' in columns:
                return 2  # V2: 包含地址字段
            else:
                return 1  # V1: 基础版本
        except Exception as e:
    ...
    def create_fts_table(self, db: Session) -> bool:
        ...
        try:
            ...
            # 创建V3版本触发器保持数据同步
            self._create_fts_triggers_v3(db)
    ...
    def rebuild_fts_table_v3(self, db: Session) -> bool:
        """
        重建FTS表到V3版本（包含AI语义字段）

        :param db: 数据库会话
        :return: 是否重建成功
        """
        try:
            self.logger.info("开始重建FTS表到V3版本...")
            ...
            # 4. 创建V3版本触发器
            self._create_fts_triggers_v3(db)
            self.logger.info("V3触发器创建完成")

            # 5. 填充所有数据（包含新字段）
            self._populate_fts_data_v3(db)

            db.commit()
            return True

        except Exception as e:
            self.logger.error(f"FTS表重建到V3失败: {e}")
            db.rollback()
            return False

    def _populate_fts_data_v3(self, db: Session):
        """
        填充V3版本FTS数据（包含完整AI语义字段）
        ...
        INSERT INTO photos_fts(
            photo_id, filename, original_path, description, location_name,
            tags_content, categories_content, analysis_content,
            scene_type, emotion, activity, location_type, detected_objects,
            people_count, time_period, confidence_score
        )
        SELECT
            p.id,
            p.filename,
            ...
            COALESCE((
                SELECT json_extract(pa.analysis_result, '$.description')
                FROM photo_analysis pa
                WHERE pa.photo_id = p.id AND pa.analysis_type = 'content'
                LIMIT 1
            ), ''),
            -- AI语义字段
            COALESCE((
                SELECT json_extract(pa.analysis_result, '$.scene_type')
                FROM photo_analysis pa
                WHERE pa.photo_id = p.id AND pa.analysis_type = 'content'
                LIMIT 1
            ), ''),
            COALESCE((
                SELECT json_extract(pa.analysis_result, '$.emotion')
                FROM photo_analysis pa
                WHERE pa.photo_id = p.id AND pa.analysis_type = 'content'
                LIMIT 1
            ), ''),
            COALESCE((
                SELECT json_extract(pa.analysis_result, '$.activity')
                FROM photo_analysis pa
                WHERE pa.photo_id = p.id AND pa.analysis_type = 'content'
                LIMIT 1
            ), ''),
            COALESCE((
                SELECT json_extract(pa.analysis_result, '$.location_type')
                FROM photo_analysis pa
                WHERE pa.photo_id = p.id AND pa.analysis_type = 'content'
                LIMIT 1
            ), ''),
            ...
            -- 新增字段
            COALESCE((
                SELECT json_extract(pa.analysis_result, '$.people_count')
                FROM photo_analysis pa
                WHERE pa.photo_id = p.id AND pa.analysis_type = 'content'
                LIMIT 1
            ), ''),
            COALESCE((
                SELECT json_extract(pa.analysis_result, '$.time_period')
                FROM photo_analysis pa
                WHERE pa.photo_id = p.id AND pa.analysis_type = 'content'
                LIMIT 1
            ), ''),
            COALESCE((
                SELECT CAST(json_extract(pa.analysis_result, '$.confidence') AS TEXT)
                FROM photo_analysis pa
                WHERE pa.photo_id = p.id AND pa.analysis_type = 'content'
                LIMIT 1
            ), '')
        ...
        result = db.execute(text(populate_sql))
        self.logger.info(f"FTS表V3数据填充完成，填充行数: {result.rowcount}")
    ...
    def _create_fts_triggers_v3(self, db: Session):
        """
        创建V3版本触发器（包含AI语义字段）
        ...
        try:
            ...
            # 分析结果更新触发器 - V3版本支持完整AI语义字段
            analysis_update_trigger_sql = """
            CREATE TRIGGER photos_fts_analysis_update AFTER INSERT ON photo_analysis
            BEGIN
                UPDATE photos_fts SET
                    analysis_content = COALESCE(json_extract(NEW.analysis_result, '$.description'), ''),
                    scene_type = COALESCE(json_extract(NEW.analysis_result, '$.scene_type'), ''),
                    emotion = COALESCE(json_extract(NEW.analysis_result, '$.emotion'), ''),
                    activity = COALESCE(json_extract(NEW.analysis_result, '$.activity'), ''),
                    location_type = COALESCE(json_extract(NEW.analysis_result, '$.location_type'), ''),
                    detected_objects = COALESCE((
                        SELECT GROUP_CONCAT(value, ' ')
                        FROM json_each(NEW.analysis_result, '$.objects')
                    ), ''),
                    people_count = COALESCE(json_extract(NEW.analysis_result, '$.people_count'), ''),
                    time_period = COALESCE(json_extract(NEW.analysis_result, '$.time_period'), ''),
                    confidence_score = COALESCE(CAST(json_extract(NEW.analysis_result, '$.confidence') AS TEXT), '')
                WHERE photo_id = NEW.photo_id;
            END
            """
            db.execute(text(analysis_update_trigger_sql))

            # 分析结果删除触发器 - V3版本清空所有AI语义字段
            analysis_delete_trigger_sql = """
            CREATE TRIGGER photos_fts_analysis_delete AFTER DELETE ON photo_analysis
            BEGIN
                UPDATE photos_fts SET
            ...
            # 分析结果更新触发器 - 处理UPDATE操作
            analysis_update_on_update_trigger_sql = """
            CREATE TRIGGER photos_fts_analysis_update_on_update AFTER UPDATE ON photo_analysis
            BEGIN
                UPDATE photos_fts SET
                    analysis_content = COALESCE(json_extract(NEW.analysis_result, '$.description'), ''),
                    scene_type = COALESCE(json_extract(NEW.analysis_result, '$.scene_type'), ''),
            ...
            END
            """
            db.execute(text(analysis_update_on_update_trigger_sql))

            self.logger.info("FTS V3触发器创建成功")

        except Exception as e:
            self.logger.error(f"创建FTS V3触发器失败: {e}")
            raise
