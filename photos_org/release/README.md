...
1. [**申请并配置免费大模型API密钥；如选用高德API解析照片地址，需申请并配置高德API密钥**](#七api密钥配置)
2. 导入照片到系统中
3. 先进行**基础分析**（免费、快速的质量评估）
4. 再进行**GPS转地址**（高德、Nominatim或离线解析）
5. 启动**人脸识别**（本地AI模型，自动聚类人物）
6. 选择性进行**AI分析**（深度内容识别，需要API密钥）
7. 使用智能搜索和筛选功能管理照片
8. 享受智能照片管理体验！
...
⚠️ **重要：** AI分析功能需要有效的阿里云DashScope千问大模型API密钥；GPS转地址可选择高德API、Nominatim或离线服务。
